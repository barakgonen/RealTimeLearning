...
int main(int argc, char **argv) {
	...
	bool isLoggerOn = stringToBool(argv[1]);
	int numberOfPointsForFiltering = stringToInt(argv[2]);
	bool isTestMode = stringToBool(argv[3]);
	bool isMulti = stringToBool(argv[4]);
	if (isTestMode) {
		std::cout << "BG test mode!" << std::endl;
	} else {
		std::cout << "\nRun mode is starting!" << std::endl;
	}
	...
	if (isTestMode) {
		PerformanceTester tester{isLoggerOn, numberOfPointsForFiltering, isMulti, argv, argc};
		tester.run();
	} else {
