...
file = open('/local/RealTimeLearning/main/bg.csv')
type(file)
...
numberOfRelevantPoints = 150
# Letting the highest 60 ranges be in red color
for i in range(numberOfRelevantPoints):
        if i <= numberOfRelevantPoints:
                print("Distance: {:.10f}".format(SortedRanges[i]), end = "")
                print(", x = {:.10f}".format(sortedXs[i]), end = "")
                print(", y = {:.10f}".format(sortedYs[i]), end = "")
                print(", z = {:.10f}".format(sortedZs[i]),)
